...
/**
 * Selector for every phrasebook field carrying per-form provenance: each
 * phrase headword AND each dialog turn line. Phrase headwords and dialog turns
 * are both marked `data-script-annotation-host` because each is a primary line
 * the learner reads; the fallback annotation shows on those lines when a
 * requested form is not directly available.
 */
...
/**
 * Apply the effective preference to one phrasebook field (a phrase headword or
 * a dialog turn line).
 *
 * - When `selectScript` returns an available form (the requested script is
 *   authored/verified), the leading text node is updated to that form and
 *   `lang` is set to `zh-Hans` (Simplified) or `zh-Hant` (Traditional)
 *   accordingly.
 ...
 * - The #251 fallback annotation is managed ONLY on annotation hosts
 *   (`data-script-annotation-host`); a requested-but-unavailable form shows it.
 *
 * Updates only the visible text, `lang`, and the annotation — never storage,
 * focus, order, the scenario filter/URL, or the phrase/dialog set.
 */
function applyField(element: HTMLElement, preference: ScriptPreference): void {
  ...
  // The annotation is a phrase-headword/dialog-turn affordance.
...
/**
 * Apply the global script preference to every phrasebook field (phrase
 * headwords and dialog turn lines).
 *
 * Behavior contract (#236/#256):
 ...
 * - Preference changes preserve the `?scenario=` filter/URL, source order,
 *   the phrase/dialog references, scroll position, and focus; no storage
 *   writes happen beyond the global preference owner.
 ...
 */
export function initPhrasebookScriptPreference(
