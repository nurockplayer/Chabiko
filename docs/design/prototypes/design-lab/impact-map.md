...
> `8d0872a6655ede4c3801538ffa93e2f34221c4f4` and independent-review repair
> candidate `cb82c28297eb0556fe33fca3855adfa572f66b22`. A complete exact-head T3
> integration run passed on `e715d250dbb9203088648b5f921440eb26bf4758`
> before those bounded repairs; their resulting head receives the same gate and
> a fresh review before final handoff.
...
| Capture and committed evidence | `scripts/capture-design-lab.ts` consumes the built loopback routes and comparison page. Reviewers and the prototype report consume the 20 individual PNGs, four comparison PNGs, `capture.json`, and `docs/design/evidence/design-lab/README.md`; the focused suite validates that committed publication directly. |
| Focused validation | `tests/design-lab-routes.test.ts` validates consumed fixture failure boundaries, layout isolation, root ownership, storage non-use, every route and surface, interaction semantics, asset provenance, grammar-specific review fixes, comparison behavior, committed and staged capture closure, drift, both publication rollback branches, and dirty-environment preservation. |
| Validation routing | `scripts/validation/classify.ts` maps the isolated fixture adapter and coupled Design Lab evidence/provenance/WebP paths to `tests/design-lab-routes.test.ts`; `tests/validation/classifier.test.ts` prevents both mappings from drifting. Isolated artifacts remain at the smallest sufficient T1 gate, while learner-visible Astro and script changes still classify the complete candidate to T3. |
| Build and final gate | Astro lint/typecheck/build consume all source routes and assets. `pnpm validate` is the final T3 consumer and includes full Vitest, build, production visual regression, accessibility, and content validation. The pre-review exact-head run is recorded below; the final repaired head receives the same gate before rereview. |
...
The committed human-owned evidence README, including exactly one pair of
generated block markers, is the capture template and a workflow prerequisite.
The capture owns only that generated block; it does not recreate the surrounding
human documentation when the entire evidence directory is missing.
...
| --- | --- |
| Clean checkout / absent managed evidence artifacts | With the committed human-owned README and its markers present, capture supports all 24 managed PNGs and `capture.json` being absent, creates its owned staging/output, and publishes only after the rebuilt artifacts plus README block validate. An entirely absent evidence directory is invalid because capture does not own or recreate the human README prose. |
...
Focused and capture results below were observed on the implementation and
post-review repair candidates. The first complete T3 result is recorded below;
the final handoff records the required exact-head rerun after all repairs.
...
| 2 | Use the same Chabiko content and core UX in every grammar. | `src/content/designLabFixture.ts`; five grammar components | Focused fixture and complete-consumer contract tests. | 69/69 focused tests passed on the repair candidate; all versions use Lesson 001, vocabulary `voc-002`, four readiness targets, path labels, reveal/rating, quiz, and continuation behavior from canonical inputs. Empty or malformed practice and malformed rendered lesson members fail closed. |
...
| 12 | Publish evidence transactionally and preserve unrelated dirty-worktree files. | `scripts/capture-design-lab.ts`; capture self-test in `tests/design-lab-routes.test.ts` | Real subprocess capture, repeated-run byte comparison, dirty sentinel, failpoints after original backup and after candidate publication, scroll drift, toolbar drift, and metadata drift tests. | Candidate validates before swap; repeated owned output is byte-identical; sentinel survives; both destructive publication failure branches restore the prior directory byte-for-byte and never publish a partial set. |
...
| 16 | Close the 24-image evidence set over dimensions, bytes, and metadata. | 20 individual PNGs; four comparison PNGs; `capture.json`; evidence README generated block; classifier mapping | Canonical capture, direct committed `validateCapturePublication()`, closed PNG inventory, and artifact-only classifier/CLI self-tests. | 20 files are exactly 390 x 844, four are exactly 2000 x 934, every SHA/dimension matches metadata, the ordered manifest digest is `9ff613552937f607cec81e8ec6b0bfd1de51ed0ec404b65dbefea4c5fb99f08e`, and later evidence/provenance/WebP-only changes cannot bypass the coupled test. |
...
| 18 | Pass the classifier-required validation on the final exact head. | Entire scoped candidate | `pnpm validate:classify`, then final exact-head `pnpm validate`. | Exact head `e715d250dbb9203088648b5f921440eb26bf4758` classified T3 and passed lint, typecheck, 2,052 Vitest tests, a 1,615-page build, 109/109 visual gates, 40/40 accessibility gates, and content validation before independent review. The final handoff records the required rerun on the post-review repair head. |
