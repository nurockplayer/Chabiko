...
If initialization finds an already-published exact empty root after an
uncertain publisher acknowledgement or parent-sync failure, adoption retains
and rechecks the root and parent identities, confirms the marker/events-only
layout with no lock or events, and fsyncs the parent directory before it may
append the initialization event or report success. A failed barrier preserves
the root and leaves the journal empty for a later retry.
...
| External output rollback retains one original descriptor, checks immutable bytes and frozen metadata, and removes only verified owned aliases | `unicode_review_external_io.ts` | `unicode-review-external-io.test.ts`: later-write in-place edits, truncation, binary/mode mutation, staged/published extra hard links, symlink collisions, and verified rollback after temporary-alias removal failure |
