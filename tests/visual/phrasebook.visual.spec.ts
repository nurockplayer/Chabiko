...
test.describe('/phrasebook/ visual baselines (full first-release surface)', () => {
  for (const visualCase of PHRASEBOOK_VISUAL_CASES) {
    test(`${visualCase.snapshotName}`, async ({ page }) => {
      ...
      // The complete first-release surface renders: all 30 phrases, all 6
      // dialogs, all 36 turns — no pending state and no eligibility gate.
...
test.describe('/phrasebook/ behavior, viewport, console', () => {
  ...
  for (const viewport of VIEWPORTS) {
    test(`toolbar + first scenario fully inside the ${viewport.width}px viewport`, async ({
      page,
    }) => {
      ...
      // The header, the filter toolbar, and the first scenario's heading are
      // all horizontally contained (no horizontal overflow at any Issue #205
      // viewport), including the native select and the count status line. The
      // whole scenario section is intentionally taller than the viewport, so a
      // single short heading is the horizontal-containment probe.
  ...
  test('console: no errors for the full phrasebook surface', async ({
    page,
  }) => {
    const errors: string[] = [];
