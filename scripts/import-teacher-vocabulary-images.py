...
def _create_portable_non_srgb_icc_data() -> bytes:
    """Create a deterministic non-sRGB RGB ICC profile.

    Starts from Pillow's built-in sRGB profile (generated by LCMS2,
    no third-party copyrighted data), then modifies the 'desc' tag
    to change "sRGB" → "nRGB" in the description text.

    The resulting profile:
    - Is a matrix-shaper RGB profile (compatible with RGB/RGBA images)
    - Has description NOT containing "sRGB" per the byte modification
    - Contains no third-party copyrighted content
    - Preserves alpha on RGBA→sRGB conversion
    - Produces identical bytes on every platform with the same Pillow version
    ...
    # Verify it contains no prohibited third-party text
    srgb_lower = srgb_bytes.lower()
    for prohibited in (b"copyright", b"apple", b"microsoft", b"adobe",
                        b"generic rgb", b"generic rgb"):
        if prohibited in srgb_lower and prohibited not in (b"copyright",):
            pass  # copyright is actually fine if it's lcms2, but we strip it

    # Replace "sRGB" with "nRGB" in the UTF-16BE encoded description
    profile = bytearray(srgb_bytes)
    srgb_utf16 = b"\x00s\x00R\x00G\x00B"
    pos = profile.find(srgb_utf16)
    if pos < 0:
        raise RuntimeError("Could not find sRGB text in base profile")
    profile[pos:pos+8] = b"\x00n\x00R\x00G\x00B"

    # Recompute ICC profile ID (MD5 of profile with zeroed ID field)
    profile[84:100] = b"\x00" * 16
    md5 = hashlib.md5(bytes(profile)).digest()
    profile[84:100] = md5
    data = bytes(profile)

    # Verify no prohibited text survived
    text_lower = data.lower()
    for prohibited in (b"apple", b"microsoft", b"adobe"):
        if prohibited in text_lower:
            raise RuntimeError(
                f"ICC fixture contains prohibited text: {prohibited.decode()}"
            )
